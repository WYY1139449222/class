...
app.use(session({
    ...
    name:'hello',//默认connect.sid
	secret: 'myqqq',//session会根据这个属性 和后端种在 session的属性名 来生成对应的字段
...
app.post('/login',function(req,res){
    ...
    if(bol){
        ...
        req.session.username = username;//后端再session上种植了一个属性
...
app.get('/info',function(req,res){
    ...
    //登录之后 后端会给前端种植一个cookie
    //以后每次请求后台接口的时候 后端都会根据这个cookie的值 去判断 前端是否处于登录有效期
    //后端的具体写法就是 根据登陆时 再session上设置的属性  根据有没有进行判断
